...
#collectors
codes = Vector{String}()
EC_values = Vector{Float64}()
vol_steel = Vector{Float64}()
vol_wood = Vector{Float64}()


count = 1
for bottomchord_material in material_options
    for topchord_material in material_options
        for bottomweb_material in material_options
            for topweb_material in material_options
                for strut_material in material_options
                    for support_material in material_options
                        ...
                        println(
                            """
                            Bottom chord: $bottomchord_material
                            Top chord: $topchord_material
                            Bottom web: $bottomweb_material
                            Top web: $topweb_material
                            Strut: $strut_material
                        ...
                        #define variables
                        x_bottom_parents = [SpatialVariable(node, 0., dxmin, dxmax, :X) for node in model.nodes[i_left_bottom[2:end-1]]]
                        x_bottom_children = [CoupledVariable(node, parent, -1.0) for (node, parent) in zip(model.nodes[i_right_bottom[2:end-1]], x_bottom_parents)]
